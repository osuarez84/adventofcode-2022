...
func executeMovements(numCrates, from, to int, lStacks []*lls.Stack) {
	temporalStack := lls.New()

	// Move from init to temporal stack
	for i := 0; i < numCrates; i++ {
		tmp, isAnything := lStacks[from].Pop()
		if isAnything {
			temporalStack.Push(tmp)
			fmt.Printf("Moving %s from %d to temporal stack\n", tmp, from)
		} else {
			fmt.Println("Stack empty, nothing to move.")
		}
	}
	fmt.Println("Temporal stack: ", temporalStack)
	fmt.Println("Temporal stack size: ", temporalStack.Size())

	// Move from temporal to the final stack
	temporalStackSize := temporalStack.Size()
	for i := 0; i < temporalStackSize; i++ {
		tmp, _ := temporalStack.Pop()
		lStacks[to].Push(tmp)
		fmt.Printf("Moving %s from temporal stack to %d\n", tmp, to)
	}
