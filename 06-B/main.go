...
func main() {
	...
	for i := 14; i <= len(runeDataStream); i++ {
		fmt.Println(string(runeDataStream[i-14:i]))
		substringCont := 0
		for _, el := range runeDataStream[i-14:i] {
			tmpCount := strings.Count(string(runeDataStream[i-14:i]), string(el))
			if tmpCount > 1 {
				break
			} else {
				substringCont++
		...
		if substringCont == 14 {
			...
			startMessage = string(runeDataStream[i-14:i])
			indexCharacter = i
			break
		}
	}
